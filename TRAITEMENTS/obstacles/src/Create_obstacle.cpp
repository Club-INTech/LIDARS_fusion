...
//    }
    for (int j=0; j<list_bounds.size(); j++) {
        obstacle.width = sqrt(pow(list_bounds[j].the_beginning_distance,2) + pow(list_bounds[j].the_end_distance,2)
                - 2 * list_bounds[j].the_beginning_distance * list_bounds[j].the_end_distance * cos(fmodf(list_bounds[j].the_end_angle - list_bounds[j].the_beginning_angle, (2*PI))));  // Al Kashi
